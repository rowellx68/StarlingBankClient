...
namespace StarlingBankClient.Tests.Helpers
{
    static class TestHelper
    {
                ...
                bool allowExtra, bool isOrdered)
        {
            ...
            var rHashSet = new HashSet<object>(rightList);
        ...
        {
            var leftDictInv = new Dictionary<string, string>(leftDict, StringComparer.CurrentCultureIgnoreCase);
            var rightDictInv = new Dictionary<string, string>(rightDict, StringComparer.CurrentCultureIgnoreCase);
        ...
        public static bool IsSameInputStream(Stream input1, Stream input2)
        {
            ...
            var ch = input1.ReadByte();
            while (ch != -1)
            {
                var ch2 = input2.ReadByte();
                if (ch != ch2)
                {
                    return false;
                }
            ...
            var input2Finished = (input2.ReadByte() == -1);
        ...
        {
            var bytes = Encoding.UTF8.GetBytes(convertme);
        ...
        {
            var lHashSet = new HashSet<T>(left);
        ...
        public static bool IsOrderedSupersetOf<T>(this IEnumerable<T> left, IEnumerable<T> right, bool checkSize = false)
        {
            ...
            while(lItr.MoveNext())
            {
                var lCurrent = lItr.Current;

                //right list ended prematurely
                if (!rItr.MoveNext())
                    return false;

                var rCurrent = rItr.Current;
